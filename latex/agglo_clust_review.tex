...
\begin{algorithm}
  \caption{Agglomerative Clustering}
   \hspace*{\algorithmicindent} \textbf{Inputs:} graph $\mathcal{G}(V,E,W)$; bool {\color{blue}\emph{addMustNotLink}}  \\
  ...
  \begin{algorithmic}[1]
    ...
    % \Procedure{SignedGraphEdgeContr}{{\color{blue}bool \emph{addConstraints}}}
      ...
      \State Set \textsc{canBeLinked}$(e)$ = True, $\forall e\in E$
      \State
      \While{PQ is \textbf{not} empty}
        \State Pop edge $e_{uv}\in E'$ with highest abs. priority $|\tilde{w}|$
        \State

        \If{({\color{ForestGreen}\textbf{$\tilde{w} > 0$}}) \textbf{and} \textsc{canBeLinked}$(e_{uv})$}
        ...
        %   \State Propagate constrained flags of double edges;
          ...
          \State Replace double edges in $\mathcal{G}'$ with single edges with updated priorities

        \EndIf
        \State
        \If{({\color{red}\textbf{$\tilde{w} \leq 0$}}) \textbf{and} {\color{blue}\emph{addMustNotLink}}}
          % \State Flag $e_{uv}$ as MustNotLink
          \State Set \textsc{canBeLinked}$(e_{uv})$ to False
        \EndIf
      \EndWhile
