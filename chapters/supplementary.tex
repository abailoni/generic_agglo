...
% Mention the main trick in the loss: ignore background pixels (so focus on boundaries). 

% Same mapping defined in \ref{eq:mappings}

% Due to the unbalanced nature of the original affinities, we had to perform a parameter-search of the $\beta$ hyper-parameter. Since with a certain value of $\beta$ could be optimal for some versions of \algname{} but not others, we decide to perform a parameter search for each version of \algname{} and Table \ref{tab:results_cityscapes_val} lists the best-case performances for each of the methods.
