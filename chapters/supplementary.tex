...
\paragraph{Extended results} For all algorithms, the chosen value of bias parameter was $\beta = 0$. \TODO{Finish}
...
%     \centering
        \begin{tabular}{l|c|c|c|c}
          \algname{} linkage & \makecell{Cremi-Score\\(higher better)}  & \makecell{Rand-Score\\(higher better)} & \makecell{VI-merge\\(lower better)} & \makecell{Runtime\\(lower better)} \\ \midrule
          % Pipeline & method & \textsc{No} & \textsc{Yes} \\ 
Average & \textbf{0.226} & \textbf{0.936} & 0.315 & 3.49 $\cdot$ 10$^4$ \\
Sum + CLC & 0.282 & 0.906 & 0.358 & 4.64 $\cdot$ 10$^4$ \\
Abs Max & 0.322 & 0.897 & 0.286 & 1.24 $\cdot$ 10$^4$ \\
Max + CLC & 0.324 & 0.893 & 0.292 & 6.31 $\cdot$ 10$^4$ \\
Sum & 0.334 & 0.872 & 0.461 & 4.74 $\cdot$ 10$^4$ \\
Average + CLC & 0.563 & 0.772 & 0.259 & 2.95 $\cdot$ 10$^4$ \\
Min & 2.522 & 0.030 & 0.197 & 2.97 $\cdot$ 10$^3$ \\
Min + CLC & 2.522 & 0.030 & 0.197 & 4.77 $\cdot$ 10$^3$ \\
Max & 2.626 & 0.028 & 7.069 & \textbf{6.04 $\cdot$ 10$^\mathbf{2}$} \\
        \end{tabular}
        \vspace*{1.1em}
    \caption{Performances achieved by different versions of \algname{} on the CREMI 2016 training set. CLC stands for cannot-link constraints}
...
% In our experiments, edge weights are estimated \UPDATE{by using a CNN that predicts how likely two neighboring pixels are to be part of the same cluster}. 
A crop of CREMI training data sample B was used.
...
\end{equation}
where  $N(x)=\mathrm{Logit}[n(x)]$; $F(x)=\mathrm{Logit}[p(x)]$ and $\mathcal{K}\in \mathbb{R}^+$ is a positive factor representing the amount of added noise. $\tilde{F}_{+}(x;\mathcal{K})$ represents then a merge-biased prediction, such that the probability for two pixels to be in the same cluster is increased only if $N(x)>0$ (see Fig. \hyperref[fig:noisy_affs]{\ref*{fig:noisy_affs}b}), whereas $\tilde{F}_{-}(x;\mathcal{K})$ is a split-biased prediction with decreased probabilities when $N(x)<0$ (Fig. \hyperref[fig:noisy_affs]{\ref*{fig:noisy_affs}c}). \TODO{Add impl. details}
