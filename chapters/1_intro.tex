...
\caption{
{\small (\textbf{a}) Some iterations of the generalized algorithm on toy graphs with attractive (green) and repulsive (red) interactions. (\textbf{b}) Example of CREMI 2016 data \cite{cremiChallenge} overlaid with instance segmentation. (\textbf{c}) Agglomeration order, representing which pairs of neighboring pixels were merged first (blue) and later on (red).
We note how similar update rules, like \emph{Sum} (in the middle) and \emph{Mean} (on the right), lead to strongly different merging orders. 
