...
\centering
    ...
    \captionof{figure}{The two figures represent the CNN predictions on a slice of the neuron segmentation CREMI challenge \cite{cremiChallenge}. (a) shows the original predictions $F(x)$, (b) the merge-biased version $\tilde{F}_{+}(x;\mathcal{K})$ defined in Eq. \ref{eq:noise_biased_predictions} and c) the split-biased version $\tilde{F}_{-}(x;\mathcal{K})$. Blue pixels represent boundary evidence.
