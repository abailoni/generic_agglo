...
\begin{table*}
    ...
    \begin{subtable}[t!]{\textwidth}\centering
        \begin{tabular}{r l || c | c | c}
            ...
            % \midrule[0.15em]
            \midrule\midrule
            Sum: & \thead[l]{$f(\tilde{\cost}_1,\tilde{\cost}_2) = \tilde{\cost}_1+\tilde{\cost}_2$} & \thead{Sum Linkage\\Hierarchical Aggl.\\Clustering} & \thead{Greedy Additive \\ Edge Contraction \\\cite{keuper2015efficient}} & \thead{Greedy \\Fixation \\\cite{levinkov2017comparative}} \\ \midrule
...
Another proposed clustering algorithm for signed graphs, named Mutex Watershed \cite{wolf2018mutex}, also introduces dynamical cannot-link constraints. In the supplementary material (Sec. \ref{sec:equivalence_MWS}) we show how it can be seen as an efficient implementation of \algname{} with the \emph{absolute-maximum} update rule defined in Table \ref{tab:linkage-criteria} and we also observe that this case the algorithm returns the same clustering with or without enforcing cannot-link constraints.

On the other hand, new variations of agglomerative clustering on signed graphs that, to our knowledge, were never mentioned in the literature are given by \algname{} with update rules \emph{mean}, \emph{max} and \emph{min} (rows 3 to 5 in Table \ref{tab:linkage-criteria}). 

In this work we test and compare these algorithms and the five update rules listed in Table \ref{tab:linkage-criteria} that represent the most common options. 
