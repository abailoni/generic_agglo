...
\subsection{\algname{}: Generalized Algorithm for Signed Graph Agglomerative Clustering} \label{sec:algorithm} 
...
\begin{table*}
    ...
    \end{subtable} 
    \caption{The table lists the existing clustering algorithms that can be reformulated as special cases of the proposed generalized algorithm \algname{}, given a linkage criteria, a type of graph (signed or unsigned) and the optional use of cannot-link constraints. The set $E_{uv}$ of all edges connecting cluster $S_u$ to cluster $S_v$ is defined as $E_{uv}=(S_u \times S_v) \cap E$.}
...
On the other hand, to our knowledge, the presented agglomerative algorithm with \emph{Average}, \emph{Max} or \emph{Min} linkage was never applied to signed graphs or combined with cannot-link constraints, \UPDATE{thus we decided to test all these new variations.} 
