...
\label{fig:edge_contraction_and_contr_graph}  }
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
\includegraphics[width=0.91\textwidth]{./figs/add_constraints.pdf}
\captionof{figure}{ 
{\small Some iterations of the generalized algorithm  with (top) and without (bottom) adding cannot-link constraints. The graph has both attractive (green) and repulsive (red) edges; cannot-link constraints are given by violet bars. At each iteration, the edge selected by the algorithm is highlighted in yellow. When cannot-link constraints are enforced, the final clustering is given by two clusters instead of only one.}
