...
\subsection{Generalized Algorithm for Signed Graphs Agglomerative Clustering (\algname)} \label{sec:algorithm} 
...
In the Supplementary material we comment on the algorithm \UPDATE{computational complexity $\mathcal{O}(?)$} and present an efficient implementation based on an edge contraction algorithm combined with a priority queue (Sec. \ref{sec:complexity}).
...
\begin{table*}
    ...
    \end{subtable} 
    \caption{The table lists the existing clustering algorithms that can be reformulated as special cases of the proposed generalized algorithm, given a linkage criteria, a type of graph (signed or unsigned) and the optional use of cannot-link constraints. The set $E_{uv}$ of all edges connecting cluster $S_u$ to cluster $S_v$ is defined as $E_{uv}=(S_u \times S_v) \cap E$.}
...
\cite{funke2018large} uses a \emph{quantile} linkage criteria by populating a histogram for each inter-cluster interaction. In the next chapters, our experiments focus on the linkage criteria listed in Table \ref{tab:linkage-criteria}, since they represent the most used ones.
