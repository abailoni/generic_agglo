...
\end{restatable}
\noindent To our knowledge, Average and Single linkage methods have never been combined with cannot-link constraints on signed graphs, so we name these algorithms \emph{HCC-Avg} and \emph{HCC-Single}. On the other hand, we prove that enforcing constraints with a Complete linkage does not change the final clustering (Lemma \ref{lemma:absMax_and_complete_property}, in Appendix).
...
Now that we have defined the algorithms and managed to characterize them from a theoretical point of view, we move to empirical evaluation. 
